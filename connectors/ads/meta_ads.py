...
def preflight(profile_name: str, token: str, target_act: str):
    ...
    accessible = set()
    for a in adaccs:
        # account_id는 숫자만, id는 act_x 형태로 올 수도 있어 둘 다 대응
    ...
    if target_act not in accessible:
        raise RuntimeError(
            ...
            f"   2) 브레인올로지 토큰으로 부담제로 계정 조회(또는 반대)처럼 매칭이 뒤바뀜\n"
...
# Insights fetch
# -----------------------
def fetch_insights_for_date(access_token: str, ad_account_id: str, ymd: str) -> dict:
    ...
    row0 = rows[0]
    try:
        spend = float(row0.get("spend") or 0.0)
    except ValueError:
        spend = 0.0

    purchases = parse_purchases_from_actions(row0.get("actions"))
    return {"date": ymd, "spend": spend, "purchases": purchases, "raw": row0}


def fetch_yesterday_insights(access_token: str, ad_account_id: str) -> dict:
    ymd = ymd_yesterday_kst()
    return fetch_insights_for_date(access_token, ad_account_id, ymd)


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--date", type=str, default="", help="YYYY-MM-DD (기본: 어제 KST)")
    parser.add_argument("--json", action="store_true", help="마지막 줄에 JSON 결과만 출력")
    args = parser.parse_args()

    target_ymd = (args.date or "").strip() or ymd_yesterday_kst()
    ...
    print(f"[INFO] Meta Ads (KST): {target_ymd} | API={META_API_VERSION}")
    print("=" * 70)
    ...
    for p in profiles:
        ...
        # ✅ 실제 인사이트 조회 (target date)
        res = fetch_insights_for_date(p["token"], p["ad_account"], target_ymd)
        ...
        print(f"  spend: {res['spend']}")
        print(f"  purchases: {res['purchases']}")
        print("-" * 70)
    ...
    # ✅ run_daily_sales_to_gsheet.py가 파싱할 마지막 줄 JSON 출력
    if args.json:
        # run_daily_sales_to_gsheet.py가 "burdenzero / brainology" 키로 받기 쉽게 맞춤
        out = {
            "date": target_ymd,
            "mapped": {
            ...
            "total": {"spend": total_spend, "purchases": total_purchases},
        }
